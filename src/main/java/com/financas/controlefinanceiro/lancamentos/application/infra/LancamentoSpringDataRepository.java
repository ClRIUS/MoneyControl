...
public interface LancamentoSpringDataRepository extends JpaRepository<Lancamentos, UUID> {
    ...
    @Query("SELECT SUM(l.valor) FROM Lancamentos l WHERE l.tipoLancamento = :tipoLancamento AND l.idUsuarioLancamento = :idUsuario")
    Double sumByTipoLancamentoAndIdUsuarioLancamento(TipoLancamento tipoLancamento, UUID idUsuario);
}
